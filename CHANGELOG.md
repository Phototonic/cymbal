...
- **`cymbal hook` family — agent integration hooks** (fixes [#23](https://github.com/1broseidon/cymbal/issues/23)). Three small, agent-agnostic subcommands that keep coding agents using cymbal instead of sliding back to raw `grep`/`find` as context grows:
  ...
  - `cymbal hook install <agent>` and `cymbal hook uninstall <agent>` wire the above into the agent's native hook points. Supported agents: `claude-code` (settings.json `PreToolUse` + `UserPromptSubmit`), `cursor` (`.cursor/rules/cymbal.md`), `windsurf` (`.windsurfrules`). `--scope user|project`, `--dry-run`. Installs are idempotent, marker-delimited, and preserve unrelated user config.
